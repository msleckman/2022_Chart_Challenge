...
drb_1990 <- raster('Data/FORESCE_data/drb_backcasting_1990.tif')
plot(drb_1990)

files <- list.files(path = 'Data/FORESCE_data/', pattern = ".tif")
rasters <- lapply(paste0('Data/FORESCE_data/', files), raster)
...
saveGIF(
  {
    for(i in c(1:nlayers(all_rasters))){
      ...
      a<-levelplot(rat, att='ID', 
                   ...
                   colorkey=F,key = myKey
      )
      
      print(a)
    
      }
  }, interval=0.8, movie.name="gif_test.gif", ani.width = 600)
